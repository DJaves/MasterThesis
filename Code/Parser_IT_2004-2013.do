...
-------------------------------------------------------------------------------*/
	...
	*Drop non-sample years
	keep if year>2003
	duplicates tag year comune provincia, gen(dupli)
	drop if dupli==1 & last_election_year==1999
	...
	*/
	...
	preserve	
		...
		*Potential varchar replacement
		...
		save "$output\using_postvarchar.dta", replace
		
	restore
	
	rename _merge _merge1
	merge m:1 year provincia comune using "$output\using_postvarchar.dta", keepusing(margin_pct gender gender_second last_election_year id elettori)
	
	
	replace _merge1=_merge if _merge1!=3
...
------------------------+-----------------------------------
        Master only (1) |    339,048       13.21       13.21
         Using only (2) |         80        0.00       13.21
            Matched (3) |  2,227,790       86.79      100.00
------------------------+-----------------------------------
                  Total |  2,566,918      100.00
...
-------------------------------------------------------------------------------*/
	...
	*Relevant outcomes and control variables
	...
	gen commessi_pc_x100000 = delitti_commessi/pop_dec_tot*100000				// All crime commited
	gen scoperti_pc_x100000 = delitti_scoperti/pop_dec_tot*100000				// All crime commited
