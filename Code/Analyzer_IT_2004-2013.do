...
/*------------------------------------------------------------------------------
    1 	Standard Model - Totale - 20% Arbitrary threshold
-------------------------------------------------------------------------------*/
...
use final_database, clear
	...
	* RDD - No Controls - 20% margin threshold
	preserve
		...
		eststo reg1_1: reg totale_pc_x100000  dummy 1.dummy#c.margin_fem c.margin_fem if delitto_name=="totale" & margin_pct<0.2
		estadd local year_FE "No"
	restore	
	...
	* RDD - Year FE - 20% margin threshold
	preserve
		...
		eststo reg1_2: reghdfe totale_pc_x100000  dummy 1.dummy#c.margin_fem c.margin_fem if delitto_name=="totale" & margin_pct<0.2, absorb(year)
		estadd local year_FE "Yes"
	restore	
	...
	*RDD - Year FE, number of electors - 20% margin threshold
	preserve
		...
		eststo reg1_3: reghdfe totale_pc_x100000  dummy 1.dummy#c.margin_fem c.margin_fem elettori if delitto_name=="totale" & margin_pct<0.2, absorb(year) 
		estadd local year_FE "Yes"
	restore
	...
	*RDD - Year FE, number of electors, female fraction - 20% margin threshold
	preserve
		...
		eststo reg1_4: reghdfe totale_pc_x100000  dummy 1.dummy#c.margin_fem c.margin_fem elettori frac_female if delitto_name=="totale" & margin_pct<0.2, absorb(year)
		estadd local year_FE "Yes"
	restore
	

	
	esttab reg1_1 reg1_2 reg1_3 reg1_4 using "$output\totale_2.tex", stats(N r2 year_FE) star(* 0.10 ** 0.05 *** 0.01) replace se
